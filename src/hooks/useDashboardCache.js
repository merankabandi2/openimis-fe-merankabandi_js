...
export const useDashboardCache = (fetchFunction, cacheKey, dependencies = []) => {
  ...
  const getCache = useCallback(() => {
    ...
    try {
      const cached = localStorage.getItem(`merankabandi_dashboard_${cacheKey}`);
      if (cached) {
  ...
  const setCache = useCallback((data) => {
    ...
    // LocalStorage cache
    try {
      localStorage.setItem(`merankabandi_dashboard_${cacheKey}`, JSON.stringify(cacheData));
    } catch (e) {
  ...
  const prefetch = useCallback((nextCacheKey) => {
    const prefetchKey = `merankabandi_dashboard_${nextCacheKey}`;
...
export const registerDashboardServiceWorker = () => {
  if ('serviceWorker' in navigator && 'SyncManager' in window) {
    navigator.serviceWorker.register('/merankabandi-dashboard-sw.js').then(registration => {
      console.log('Merankabandi Dashboard Service Worker registered');

      // Register periodic sync for dashboard data
      if ('periodicSync' in registration) {
        registration.periodicSync.register('merankabandi-dashboard-sync', {
...
class DashboardDB {
  constructor() {
    this.dbName = 'MerankabandiDashboardCache';
    this.version = 1;
    this.db = null;
  }
