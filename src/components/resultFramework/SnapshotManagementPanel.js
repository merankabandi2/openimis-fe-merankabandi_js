...
function SnapshotManagementPanel({
  ...
  const headers = () => [
    'merankabandi.resultFramework.snapshot.name',
    'merankabandi.resultFramework.snapshot.createdDate',
    'merankabandi.resultFramework.snapshot.createdBy',
    'merankabandi.resultFramework.snapshot.status',
    '',
  ];
  ...
  const itemFormatters = () => [
    ...
    (snapshot) => (
      <div style={{ textAlign: 'right' }}>
        <Tooltip title={formatMessage(intl, 'merankabandi', 'snapshot.view')}>
        ...
        {snapshot.status === 'DRAFT' && (
          <Tooltip title={formatMessage(intl, 'merankabandi', 'snapshot.finalize')}>
  ...
  const handleFinalizeSnapshot = (snapshot) => {
    coreConfirm(
      formatMessage(intl, 'merankabandi', 'snapshot.finalize.confirm.title'),
      formatMessage(intl, 'merankabandi', 'snapshot.finalize.confirm.message'),
  ...
  return (
    <>
        ...
        <Grid container className={classes.paperHeader}>
          <Grid item xs={8}>
            <Typography variant="h6">
              {formatMessage(intl, 'merankabandi', 'resultFramework.snapshots.title')}
            </Typography>
          </Grid>
          <Grid item xs={4} style={{ textAlign: 'right' }}>
            ...
            >
              {formatMessage(intl, 'merankabandi', 'snapshot.create.button')}
            </Button>
          </Grid>
        </Grid>
        <Searcher
          ...
          tableTitle={formatMessage(intl, 'merankabandi', 'resultFramework.snapshots.list')}
          headers={headers}
      ...
      <Dialog open={createDialogOpen} onClose={() => setCreateDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>
          {formatMessage(intl, 'merankabandi', 'snapshot.create.title')}
        </DialogTitle>
        <DialogContent>
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <TextField
                label={formatMessage(intl, 'merankabandi', 'snapshot.name')}
            ...
            <Grid item xs={12}>
              <TextField
                label={formatMessage(intl, 'merankabandi', 'snapshot.description')}
            ...
            <Grid item xs={6}>
              <TextField
                label={formatMessage(intl, 'merankabandi', 'snapshot.dateFrom')}
                type="date"
            ...
            <Grid item xs={6}>
              <TextField
                label={formatMessage(intl, 'merankabandi', 'snapshot.dateTo')}
                type="date"
        ...
        <DialogActions>
          ...
          >
            {formatMessage(intl, 'merankabandi', 'dialog.create')}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={viewDialogOpen} onClose={() => setViewDialogOpen(false)} maxWidth="lg" fullWidth>
        <DialogTitle>
          {selectedSnapshot?.name} - {formatMessage(intl, 'merankabandi', 'snapshot.details')}
        </DialogTitle>
        <DialogContent>
          {selectedSnapshot && (
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <Typography variant="body2" color="textSecondary">
                  {formatMessage(intl, 'merankabandi', 'snapshot.createdDate')}: {formatDateTimeFromISO(modulesManager, intl, selectedSnapshot.snapshotDate)}
                </Typography>
        ...
        <DialogActions>
          <Button onClick={() => setViewDialogOpen(false)}>
            {formatMessage(intl, 'merankabandi', 'dialog.close')}
          </Button>
        </DialogActions>
      </Dialog>
...
const mapStateToProps = (state) => ({
  ...
  fetchingSnapshots: state.merankabandi.fetchingResultFrameworkSnapshots,
  snapshots: state.merankabandi.resultFrameworkSnapshots,
  snapshotsPageInfo: state.merankabandi.resultFrameworkSnapshotsPageInfo,
});
