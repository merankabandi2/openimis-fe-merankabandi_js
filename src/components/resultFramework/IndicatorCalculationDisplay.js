    ...
    <Paper className={classes.indicatorRow}>
      <Grid container spacing={2} alignItems="center">
        ...
        <Grid item xs={6} md={1} style={{ textAlign: 'right' }}>
          ...
          {showDetails && (
            <Tooltip title={formatMessage(intl, 'merankabandi', 'indicator.details')}>
        ...
        {calculationResult && (
          <Grid item xs={12}>
            <Chip
              ...
              label={formatMessage(intl, 'merankabandi', `calculation.type.${calculationResult.calculationType}`)}
              size="small"
      ...
      {showDetails && expanded && calculationResult && (
        <Accordion expanded={expanded} onChange={() => setExpanded(!expanded)}>
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Typography>{formatMessage(intl, 'merankabandi', 'indicator.calculationDetails')}</Typography>
          </AccordionSummary>
          <AccordionDetails>
            <Table size="small" className={classes.detailsTable}>
              <TableBody>
                {calculationResult.calculationType === 'MIXED' && (
                  <>
                    <TableRow>
                      <TableCell>{formatMessage(intl, 'merankabandi', 'indicator.systemValue')}</TableCell>
                      <TableCell align="right">{calculationResult.systemValue || 0}</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell>{formatMessage(intl, 'merankabandi', 'indicator.manualValue')}</TableCell>
                ...
                {calculationResult.genderBreakdown && (
                  <TableRow>
                    <TableCell>{formatMessage(intl, 'merankabandi', 'indicator.genderBreakdown')}</TableCell>
                    <TableCell align="right">
                ...
                {dateFrom && (
                  <TableRow>
                    <TableCell>{formatMessage(intl, 'merankabandi', 'indicator.dateFrom')}</TableCell>
                    <TableCell align="right">{formatDateFromISO(dateFrom)}</TableCell>
                  </TableRow>
                )}
                {dateTo && (
                  <TableRow>
                    <TableCell>{formatMessage(intl, 'merankabandi', 'indicator.dateTo')}</TableCell>
