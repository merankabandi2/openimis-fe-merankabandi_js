...
function MonetaryTransferUploadDialog({
    ...
    if (selectedFile) {
      ...
      if (!validTypes.includes(selectedFile.type)) {
        coreAlert(
          ...
          formatMessage(intl, 'merankabandi', 'monetaryTransfer.upload.selectValidFile')
        );
        return;
      }
  ...
  const downloadTemplate = async () => {
    try {
      ...
      coreAlert(
        formatMessage(intl, 'merankabandi', 'monetaryTransfer.upload.templateError'),
        error.message
      );
    }
  };
  ...
  const handleUpload = async () => {
    if (!file) {
      coreAlert(
        formatMessage(intl, 'merankabandi', 'monetaryTransfer.upload.noFile'),
    ...
    try {
      ...
      coreAlert(
        formatMessage(intl, 'merankabandi', 'monetaryTransfer.upload.error'),
        error.message
      );
    } finally {
      setUploading(false);
    ...
    <>
      ...
      >
            ...
            {uploading && (
              <Grid item xs={12} className={classes.progressContainer}>
                ...
                <Typography variant="body2" align="center" style={{ marginTop: 8 }}>
                  {formatMessage(intl, 'merankabandi', 'monetaryTransfer.upload.uploading')}
                </Typography>
              </Grid>
            )}
                    ...
                    {uploadResult.failed > 0 && uploadResult.invalid_items && (
                      <div className={classes.errorList}>
                        <Typography variant="subtitle2" style={{ marginTop: 16 }}>
                          {formatMessage(intl, 'merankabandi', 'monetaryTransfer.upload.errors')}:
                        </Typography>
        ...
        <DialogActions>
          ...
          >
            {formatMessage(intl, 'merankabandi', 'monetaryTransfer.upload.import')}
          </Button>
        </DialogActions>
      </Dialog>
    </>
