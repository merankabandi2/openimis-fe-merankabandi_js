...
import CardGenerationButton from './CardGenerationButton';
...
function BenefitPlanProvincesPanel({
  ...
  const itemFormatters = (params) => {
    ...
    // Card generation column
    values.push((location) => <CardGenerationButton location={location} formatMessage={formatMessage} />);
    // Payroll generation column
    values.push((location) => (location.countActive > 0
      ? <GeneratePayrollDialog location={{ ...location, benefitPlanId: benefitPlan.id }} buttonLabel={formatMessage('payroll.generate')} />
      : null));
...
const mapStateToProps = (state) => ({
  fetchingBenefitPlanProvinces: state.merankabandi.fetchingBenefitPlanProvinces,
  fetchedBenefitPlanProvinces: state.merankabandi.fetchedBenefitPlanProvinces,
  errorBenefitPlanProvinces: state.merankabandi.errorBenefitPlanProvinces,
  benefitPlanProvinces: state.merankabandi.benefitPlanProvinces,
  benefitPlanProvincesPageInfo: state.merankabandi.benefitPlanProvincesPageInfo,
  provincesTotalCount: state.merankabandi.benefitPlanProvincesTotalCount,
});
