...
import { fetchBenefitPlans, deleteBenefitPlan } from '../../social-protection-actions';
import BenefitPlanFilter from './MerankabaniBenefitPlanFilter';

// Rights from upstream socialProtection module
const RIGHT_BENEFIT_PLAN_UPDATE = 160003;
const RIGHT_BENEFIT_PLAN_DELETE = 160004;

function MerankabaniBenefitPlanSearcher({
  intl,
  modulesManager,
  history,
  rights,
  ...
  const headers = () => {
    ...
    // Wizard column
    if (rights.includes(RIGHT_BENEFIT_PLAN_SEARCH)) {
      h.push('');
    }
    return h;
  };
  ...
  const itemFormatters = () => {
    ...
    if (rights.includes(RIGHT_BENEFIT_PLAN_SEARCH)) {
      formatters.push((benefitPlan) => (
        <Tooltip title={formatMessage(intl, MODULE_NAME, 'wizard.launchButton')}>
          <IconButton
            onClick={(e) => {
              e.stopPropagation();
...
export default withHistory(
  withModulesManager(injectIntl(connect(mapStateToProps, mapDispatchToProps)(MerankabaniBenefitPlanSearcher))),
);
