...
class SectionPicker extends Component {
  ...
  nullDisplay = this.props.nullLabel || formatMessage(this.props.intl, 'merankabandi', 'section.picker.none');
  ...
  render() {
    ...
    const pickerLabel = label || formatMessage(intl, 'merankabandi', 'section.label');
    ...
    return (
      <Autocomplete
        ...
        reset={reset}
        noOptionsText={formatMessage(intl, 'merankabandi', 'section.picker.noOptions')}
      />
    );
  }
}
...
const mapStateToProps = (state) => ({
  sections: state.merankabandi.sections,
  fetchingSections: state.merankabandi.fetchingSections,
  fetchedSections: state.merankabandi.fetchedSections,
});
